...
# Helpers de gráfico
# ======================================================
def x_axis(enc: str, x_title: str):
...
    # Eixo Y sempre com legenda PREÇO conforme solicitação
    return alt.Y(enc, axis=alt.Axis(format=".0f", labelFontWeight="bold", labelColor=CINZA_TXT, title="PREÇO"),
                 scale=alt.Scale(domain=domain) if domain is not None else alt.Undefined)

def barras_com_tendencia(df: pd.DataFrame, x_col: str, y_col: str, x_type: str, titulo: str, nota: str, sort=None, y_max: Optional[int]=None, x_title: str=""):
    # prepara rótulo com milhar pontuado
    df = df.copy()
    df["_LABEL"] = df[y_col].apply(fmt_pontos)

    x_axis_obj = x_axis(f"{x_col}:{x_type}", x_title)
    y_axis_obj = y_axis(f"{y_col}:Q", domain=[0, y_max] if y_max else None)

    base = alt.Chart(df).encode(
        x=(x_axis_obj.sort(sort) if sort is not None else x_axis_obj),
        y=y_axis_obj,
...
        x=x_axis_obj if sort is None else x_axis_obj.sort(sort),
        y=y_axis_obj,
...
    # Linha de tendência (loess) - PONTILHADA, CINZA ESCURO
    line = (
        alt.Chart(df)
        .transform_loess(x_col, y_col, bandwidth=0.6)
        .mark_line(color="#666666", strokeDash=[4, 4], opacity=0.9) # Cor e pontilhado aplicados
        .encode(x=x_axis_obj, y=y_axis("loess:Q", domain=[0, y_max] if y_max else None))
    )
...
        x=x_axis("TRECHO:N", x_title="TRECHO"), # Eixo X sem título por padrão em barras empilhadas
        y=alt.Y("COUNT:Q", stack="normalize", title=""), # Eixo Y sem título em normalizado
...
    # RÓTULO PRETO P/ GOL/AZUL, BRANCO P/ LATAM, CENTRALIZADO DENTRO DO SEGMENTO, NEGRITO
...
        # Condição para cor do rótulo: BRANCO para LATAM, PRETO para os outros
        color=alt.condition(
...
    # Remoção da coluna "Nº" conforme solicitado. O índice 0-19 permanece, mas não é exibido por padrão.
...
        x_title="HORA", # Legenda Eixo Horizontal Gráfico 1
    )
